...
# Add the week_3/src directory to Python path for imports
week3_src_path = os.path.join(
...
def change_working_directory():
    """Change working directory to week_3/src for tests."""
    original_cwd = os.getcwd()
    # We're already in week_3, so src is in the current directory
    week3_src_path = "src"
    if os.path.exists(week3_src_path):
        os.chdir(week3_src_path)
    yield
...
class TestSessionManagement:
    ...
    def test_session_cleanup(self):
        ...
        # Call cleanup
        # close_session is async in Week 3
        import asyncio
