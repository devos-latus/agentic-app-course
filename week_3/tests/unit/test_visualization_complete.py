...
from agents_.visualization_tools import (
    # Core functions
...
class TestAgentIntegration(unittest.TestCase):
    ...
    def test_agent_import(self):
        ...
        try:
            from agents_.csv_agents import visualization_agent

            # Agent should exist
            self.assertIsNotNone(visualization_agent)
            self.assertEqual(visualization_agent.name, "VisualizationAgent")

            # In Week 3, visualization agent uses MCP coordination, not direct tools
            # This is expected - agent coordinates with MCP server for visualization
            self.assertEqual(len(visualization_agent.tools), 0, "Week 3 visualization agent should use MCP coordination")

            # Agent should have proper instructions for MCP integration
            self.assertIn("MCP", visualization_agent.instructions)
            self.assertIn("stdio", visualization_agent.instructions)
