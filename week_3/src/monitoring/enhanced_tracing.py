"""
Enhanced Phoenix Tracing for Week 3 - Container Version

Self-contained Phoenix tracing module adapted for containerized deployment.
This combines Week 2 tracing functionality without external dependencies.

Key concepts:
- Hierarchical span organization
- Business context as parent spans
- Auto-instrumentation as child spans
- Container-ready Phoenix integration
- Self-contained helper imports

Use cases:
- Production monitoring in containerized environments
...
@asynccontextmanager
async def trace_user_conversation(session_id: str, message_number: int, user_input: str):
    """
    ...
    │   ├── 🛠️ MCP Tool Execution (auto-instrumented)
    ...
    with tracer.start_as_current_span(
        ...
        attributes={
            ...
            "conversation.input_preview": user_input[:100] + "..." if len(user_input) > 100 else user_input,
            "service.name": "csv_analytics_container",
            "service.version": "3.0.0",
            "span.kind": "conversation",
            "otel.library.name": "enhanced_chat_service",
            "mcp.integration": "stdio_transport"
        }
    ) as conversation_span:
        
        try:
            yield conversation_span
            conversation_span.set_status(Status(StatusCode.OK, "Conversation completed successfully"))
            
        except Exception as e:
            conversation_span.set_status(Status(StatusCode.ERROR, f"Conversation failed: {str(e)}"))
...
async def trace_agent_processing(parent_span, agent_name: str, operation: str):
    ...
    with tracer.start_as_current_span(
        ...
        try:
            ...
            processing_span.set_status(Status(StatusCode.OK, f"Agent {agent_name} completed {operation}"))
            
        except Exception as e:
            processing_span.set_status(Status(StatusCode.ERROR, f"Agent processing failed: {str(e)}"))
...
@asynccontextmanager
async def trace_mcp_operation(tool_name: str, arguments: Dict[str, Any]):
    """
    Create a span for MCP tool operations.
    ...
    span_name = f"mcp_tool_{tool_name}"
    
    with tracer.start_as_current_span(
        span_name,
        kind=SpanKind.CLIENT,  # This is a client call to MCP server
        attributes={
        ...
        try:
            yield mcp_span
            mcp_span.set_status(Status(StatusCode.OK, f"MCP tool {tool_name} completed successfully"))
            
        except Exception as e:
            mcp_span.set_status(Status(StatusCode.ERROR, f"MCP tool failed: {str(e)}"))
            mcp_span.set_attribute("error.tool", tool_name)
            mcp_span.set_attribute("error.message", str(e))
            raise


def add_conversation_result(span, success: bool, response: str, agent_name: str, metadata: Optional[Dict[str, Any]] = None):
    """
...
def add_mcp_result(span, success: bool, tool_name: str, result: Dict[str, Any]):
    """
    Add MCP tool result information to the span.
    
    Args:
        span: OpenTelemetry span
        success: Whether the MCP tool call was successful
    ...
    if "error" in result:
        span.set_attribute("mcp.error", str(result["error"]))
    
    if "message" in result:
        span.set_attribute("mcp.result_message", str(result["message"]))
...
def _classify_interaction(response: str) -> str:
    ...
    if any(word in response_lower for word in ["chart", "plot", "graph", "visualization"]):
        return "visualization"
    elif any(word in response_lower for word in ["analysis", "statistics", "calculate", "average"]):
        return "analysis"
    elif any(word in response_lower for word in ["error", "cannot", "unable", "failed"]):
        return "error_handling"
    elif any(word in response_lower for word in ["table", "columns", "data", "dataset"]):
        return "data_exploration"
    elif any(word in response_lower for word in ["mcp", "tool", "server"]):
